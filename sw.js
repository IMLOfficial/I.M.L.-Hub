...
const BG_SCRIPT='<script src="./video-inspired-bg.js?v=30"></script>';
const WIDGET_SCRIPTS=[
  '<script src="./video-library.js?v=30" defer></script>',
  '<script src="./audio-library.js?v=30" defer></script>',
  '<script src="./language-widget.js?v=30" defer></script>'
];

function withWidgets(html){
  const withoutBg=html.replace(/<script\s+src="\.\/video-inspired-bg\.js\?v=\d+"\s*><\/script>/g,"");
  const cleaned=withoutBg.replace(/<script\s+src="\.\/(video-library|audio-library|language-widget)\.js\?v=\d+"\s*(defer)?\s*><\/script>/g,"");
...
self.addEventListener("fetch",event=>{
  ...
  const widgetFile=["video-library.js","language-widget.js","audio-library.js","video-inspired-bg.js"].find(file=>path.endsWith(`/${file}`));
  if(widgetFile){
